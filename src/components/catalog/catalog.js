import React from 'react';
import RestaurantCard from '../restaurants/restaurants';

function SkeletonCard() {
  return (
    <div className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm">
      <div className="h-48 bg-gray-100 animate-pulse" />
      <div className="p-4 space-y-3">
        <div className="h-5 bg-gray-100 rounded-full animate-pulse w-3/4" />
        <div className="flex gap-1 mt-1">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="w-3.5 h-3.5 bg-gray-100 rounded animate-pulse" />
          ))}
          <div className="h-3.5 bg-gray-100 rounded-full animate-pulse w-8 ml-2" />
        </div>
        <div className="h-3 bg-gray-100 rounded-full animate-pulse w-1/2" />
        <div className="h-3 bg-gray-100 rounded-full animate-pulse w-4/5" />
        <div className="h-3 bg-gray-100 rounded-full animate-pulse w-3/5" />
        <div className="flex gap-1.5 mt-2">
          <div className="h-5 bg-gray-100 rounded-full animate-pulse w-16" />
          <div className="h-5 bg-gray-100 rounded-full animate-pulse w-20" />
          <div className="h-5 bg-gray-100 rounded-full animate-pulse w-14" />
        </div>
        <div className="h-14 bg-rose-50 rounded-xl animate-pulse mt-2" />
      </div>
    </div>
  );
}

function EmptyState({ parsedQuery }) {
  return (
    <div className="text-center py-20">
      <span className="text-5xl block mb-4">🔍</span>
      <h3 className="font-semibold text-gray-900 text-lg">No results found</h3>
      <p className="text-gray-500 text-sm mt-2 max-w-xs mx-auto leading-relaxed">
        {parsedQuery
          ? `We couldn't find "${parsedQuery.yelpTerm}" nearby. Try different wording or a wider location.`
          : 'Try a different search — different wording or a wider location usually helps.'}
      </p>
    </div>
  );
}

export default function BusinessList({ businesses, isLoading, hasSearched, parsedQuery }) {
  if (isLoading) {
    return (
      <div>
        <div className="flex items-baseline gap-3 mb-6">
          <div className="h-6 bg-gray-100 rounded-full animate-pulse w-52" />
          <div className="h-4 bg-gray-100 rounded-full animate-pulse w-16" />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {Array.from({ length: 6 }).map((_, i) => <SkeletonCard key={i} />)}
        </div>
      </div>
    );
  }

  if (!hasSearched) return null;

  if (!businesses.length) return <EmptyState parsedQuery={parsedQuery} />;

  return (
    <div className="animate-fade-in">
      {/* Results header */}
      <div className="flex items-baseline gap-3 mb-6">
        <h2 className="text-xl font-bold text-gray-900">
          {parsedQuery
            ? `Places that match your vibe`
            : 'Results'}
        </h2>
        <span className="text-sm text-gray-400">{businesses.length} found</span>
      </div>

      {/* Cards grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {businesses.map((business) => (
          <RestaurantCard
            key={business.id}
            business={business}
            parsedQuery={parsedQuery}
          />
        ))}
      </div>
    </div>
  );
}
